...
- Ambos buscan dar cuenta de los mecanismos de influencia recíproca entre variables.

- La diferencia: manera en la que tratan la estabilidad temporal:
   - CLMP asume que la estabilidad temporal está siendo estimada a través de la 
   inclusión de los parámetros autorregresivos, como si todos los individuos estuvieran variando
   ...
   - El RI da cuenta de dos tipos de estabilidad o varianza:
     ...
     en el periodo de medición y que dan cuenta de las diferencias entre los individuos de
     la muestra.
...
También vamos a tener como punto de partida el paper de Roberto, Marìa y Daniel,
que presentó Roberto en la primera parte del curso.

En este paper se está testeando distintas hipótesis respecto a la relación entre la participaciòn polìtica online y la participaciòn offline (en acciones colectivas).
...
el modelo básico de Random Intercept.
...
- los componentes within están correlacionados entre si en cada ola.
- en la ola 1, desde la ola 2 estan correlacionados los residuos.

- los residuos $u_{it}$ y $v_{it}$ están normalmente distribuidos
...
La estimación se basa en la sintaxis de modelo de lavaan y sus distintos tipos de fórmula.
...
Breve descripción de los datos que estamos usando
...
Las cargas factoriales son fijadas en 1 porque RI-CLPM se basa en el supuesto de que los interceptos aleatorios tienen igual influencia en cada ocasión. Se asume que los interceptos aleatorios están correlacionados entre si. 
...
- Crosslag: Efecto de una variable independiente en t sobre la dependiente en t+1, o el recíproco.
...
Primera ola exógena (nota mental):
...
Acompañado por nuestras hipótesis, asumimos que los interceptos aleatorios están correlacionados.

No podemos asumir que los RI están correlacionados con la primera ola.
Las observaciones por lo general empiezan en un punto arbitrario del tiempo durante un proceso en desarrollo, por lo que no existen razones para asumir que están relacionados.


Como pueden ver, si bien la sintaxis es larga, cada parte sigue al pie de la letra
el esquema estadístico que planteo roberto en la primera parte. 
...
ML podríamos especificar otros estimadores alternativos. Algunos de ellos exigen datos completos, depende de nuestra estrategia para tratar los casos perdidos. Uso MLR para usar EE robustos. 
...
- Estabilidad en el tiempo de los efectos
  - No sabemos si se sostiene este supuesto de estabilidad. Estrategia específica.
...
En ese caso, no existen grandes diferencias entre estimar un CLPM y un RI. Obtendríamos resultados bastantes similares. 
...
Cuanto de la desviación pasa de una ola a la siguiente. Cambio intraindividual, habiendo controlado por las diferencias individuales estables.
